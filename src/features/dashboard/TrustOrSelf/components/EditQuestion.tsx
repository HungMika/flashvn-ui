...
import { resetCounts } from '@/features/dashboard/TrustOrSelf/api/api';
...
export default function EditQuestionPopup({
  ...
  const handleResetClick = async () => {
    try {
      const resetQuestion = await resetCounts(editQuestion._id);
      setEditQuestion(resetQuestion);
    } catch (error) {
      ...
      <div className="popup bg-[#F3F4F6] rounded-lg w-full max-w-[90%] sm:max-w-md max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="formContainer p-3 sm:p-5">
          <form onSubmit={handleSubmit}>
            <label className="formLabel block text-xs sm:text-sm font-medium mb-1 sm:mb-2 text-gray-700">Mã tình huống</label>
            <input
              type="text"
            ...
            <input
              ...
              onChange={(e) => setEditQuestion({ ...editQuestion, trustCount: parseInt(e.target.value) })}
