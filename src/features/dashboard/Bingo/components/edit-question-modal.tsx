...
//API services
import { updateQuestion } from '@/features/dashboard/Bingo/api/question';
...
export const EditQuestionModal = ({ question, keywords, onClose, onQuestionUpdated }: EditQuestionModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    const filteredWrongAnswers = wrongAnswers.map((ans) => ans.trim()).filter((ans) => ans !== '');

    if (!trimmedTitle || !trimmedCorrectAnswer || filteredWrongAnswers.length === 0 || !selectedKeyword) {
    ...
    <Dialog open={!!question} onOpenChange={onClose}>
      <DialogContent className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-lg">
        ...
        <div className="space-y-4 mt-2">
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter question..." />
        ...
        <DialogFooter className="mt-6">
          <Button disabled={loading} onClick={handleSubmit} className="font-semibold text-md py-2">
            {loading ? 'Saving...' : 'Save'}
          </Button>
        </DialogFooter>
      </DialogContent>
