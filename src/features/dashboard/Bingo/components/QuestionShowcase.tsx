...
export const QuestionShowcase = ({ questions, onClose }: QuestionShowcaseProps) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [showAnswer, setShowAnswer] = useState(false);
  const [shuffledOptions, setShuffledOptions] = useState<string[]>([]);
  const [keywordsSeen, setKeywordsSeen] = useState<string[]>([]);
  const [ConfirmDialog, confirm] = useConfirm('Close Presentation?', 'You cannot review checked keywords.');
  ...
  useEffect(() => {
    ...
    setShowAnswer(false);
    setKeywordsSeen((prev) => (prev.includes(currentQuestion.keyword) ? prev : [...prev, currentQuestion.keyword]));
  }, [currentIndex]);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 bg-[#fcefd4] p-6 flex flex-col items-center justify-center text-center overflow-auto">
        ...
        <div className="mb-4">
          <img src="/FLASH-logo-colorful.png" alt="FLASH logo" className="w-30 mx-auto" />
        </div>
                ...
                return (
                  <div
                    ...
                    className={`border px-4 py-2 rounded-lg text-left transition-all cursor-pointer ${
                      showAnswer && isCorrect ? 'border-green-500 font-semibold' : 'border-blue-200'
                    }`}
                  >
